...
(function() {
    ...
    function setupEditorEventListeners() {
        ...
        if (editorUi.importModelBtn) editorUi.importModelBtn.addEventListener('click', () => showEditorMessageBox("Model import (GLTF, OBJ, FBX) is a planned feature.", 'info', 2500));
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const editorUiElementsToCache = {
            ...
            importModelBtn: 'importModelBtn', renderCanvas: 'renderCanvas', viewTranslateBtn: 'viewTranslateBtn',
        ...
        if (editorUi.importDxfBtn) {
            const dxfFileInput = document.createElement('input');
            dxfFileInput.type = 'file';
            dxfFileInput.id = 'dxfFileInput'; // Add ID for testability
            dxfFileInput.accept = '.dxf';
            dxfFileInput.style.display = 'none';
            document.body.appendChild(dxfFileInput);

            editorUi.importDxfBtn.addEventListener('click', () => {
                dxfFileInput.click();
            });

            dxfFileInput.addEventListener('change', (event) => {
                const file = event.target.files[0];
                if (!file) return;

                showEditorMessageBox(`Importing ${file.name}...`, 'info');
                if (editorUi.loadingOverlay) editorUi.loadingOverlay.classList.add('show');

                const reader = new FileReader();
                reader.onload = (e) => {
                    const fileContent = e.target.result;

                    // The font is required by three-dxf
                    new THREE.FontLoader().load(
                        ...
                        (font) => {
                            try {
                                ...
                                if (dxf) {
                                    // Create a dummy div for the viewer
                                    const dummyContainer = document.createElement('div');
                                    const cadCanvas = new ThreeDxf.Viewer(dxf, dummyContainer, 0, 0, font);

                                    const modelGroup = new THREE.Group();

                                    // three-dxf creates its own scene. We need to extract the objects.
                                    const children = [...cadCanvas.scene.children];
                                    children.forEach(child => {
                                        if (child.isMesh || child.isLine) {
                                            // Reparent the object to our group
                                            modelGroup.add(child);
                                        }
                                    });

                                    // Convert materials to something that responds to light
                                    modelGroup.traverse(child => {
                                        if (child.isMesh) {
                                            ...
                                            child.userData.type = 'DXF_Mesh';
                                            child.name = `${file.name}_mesh`;
                                            editorObjects.push(child);
                                        } else if (child.isLine) {
                                            child.material = new THREE.LineBasicMaterial({
                                                color: child.material.color
                                            });
                                            child.userData.type = 'DXF_Line';
                                            child.name = `${file.name}_line`;
                                            editorObjects.push(child);
                                        }
                                    });
                                    ...
                                    modelGroup.name = file.name.replace(/\.[^/.]+$/, "");
                                    modelGroup.userData.type = 'DXFImport';

                                    editorScene.add(modelGroup);
                                    updateEditorObjectList();
                                    editorSaveState();
                                    showEditorMessageBox(`Successfully imported ${file.name}`, 'success');
                                } else {
                                    throw new Error('DXF data could not be parsed.');
                                }
                            } catch (err) {
                                console.error('Error importing DXF:', err);
                                showEditorMessageBox(`Failed to import DXF: ${err.message}`, 'error', 5000);
                            } finally {
                                if (editorUi.loadingOverlay) editorUi.loadingOverlay.classList.remove('show');
                                dxfFileInput.value = ''; // Reset file input
                            }
                        },
                        // onProgress callback
                        (xhr) => {
                        ...
                        // onError callback
                        (err) => {
                            console.error('An error happened while loading the font for DXF import.', err);
                            showEditorMessageBox('Could not load required font for DXF import.', 'error', 5000);
                            if (editorUi.loadingOverlay) editorUi.loadingOverlay.classList.remove('show');
                            dxfFileInput.value = '';
                        }
                    );
                ...
                reader.onerror = (err) => {
                    ...
                    showEditorMessageBox('Could not read the selected file.', 'error');
                    if (editorUi.loadingOverlay) editorUi.loadingOverlay.classList.remove('show');
                };
